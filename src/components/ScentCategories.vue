...
<template>
  <section id="families" class="section bg-canvas">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <!-- Header -->
      <div
        class="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-14"
        data-aos="fade-up"
      ...
      >
        ...
        <p class="text-sm sm:text-base text-muted max-w-md leading-relaxed">
          Pokoknya aroma itu punya kisah di belakangnya. Ayo cari tau sendiri
          apa Fragrance Family favoritmu—cukup cari, temukan dan rasakan
          sendiri.
        </p>
      </div>
      ...
      >
        ...
        >
          ...
          <!-- Content -->
          <div
            class="absolute inset-0 p-4 sm:p-5 flex flex-col justify-end text-canvas"
          >
            <p
              class="text-[10px] tracking-[0.22em] uppercase opacity-80 mb-1.5"
            >
              {{ countByFamily(family.id) }}
              {{ countByFamily(family.id) === 1 ? "fragrance" : "fragrances" }}
            </p>
